...
const SortableItemComponent = ({
  ...
  const onGestureEvent = useAnimatedGestureHandler({
    ...
    onActive: (event) => {
      ...
      const offsetY =
        Math.round(y.value / heightShared.value) * heightShared.value;
      offsets.forEach((o, i) => {
    ...
    onEnd: (event) => {
      ...
      if (
        offsetY >= offsets.length * heightShared.value &&
        offsetY < offsets.length * heightShared.value + 100
      ) {
  ...
  const style = useAnimatedStyle(() => ({
    ...
    opacity: shouldRemove.value ? 0 : 1,
    height: withTiming(shouldRemove.value ? 0 : heightShared.value),
    transform: [
  ...
  return (
    <PanGestureHandler {...{ onGestureEvent }} enabled={editingHome}>
      <Animated.View
        style={[
          {
            position: "absolute",
